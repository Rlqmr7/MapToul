...
namespace
{
    const int MAP_WIDTH = 20;   // 例: 40に拡張
    const int MAP_HEIGHT = 20;  // 例: 40に拡張
...
private:
	...
	Point scrollOffset_; // ← スクロールオフセットを追加
	bool multiDeleteMode_ = false; // 複数削除モード
	std::vector<Point> selectedForDelete_; // 削除対象座標
};